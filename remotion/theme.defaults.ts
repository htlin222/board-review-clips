// Built-in defaults for every tunable knob. This is the fallback baseline:
// config.toml (at the repo root) overrides any subset of these, and `pnpm
// sync-config` bakes those overrides into config.generated.json, which theme.ts
// merges on top of these values. Edit config.toml to customize — the comments
// there quote each default so you can always restore it. Touch this file only
// when adding a brand-new knob.
